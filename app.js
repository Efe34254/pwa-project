...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function sendAppNotification(title, body) {
        console.log(`[App] Sending notification: "${title}" - "${body}"`);

        if (localStorage.getItem('notifications') !== 'true') {
            console.log('[App] Notifications suppressed (User setting is off).');
            return;
        }

        if (Notification.permission !== 'granted') {
            console.warn('[App] Cannot send notification: Permission is ' + Notification.permission);
            return;
        }

        // 1. Try Service Worker (Preferred for PWA)
        if ('serviceWorker' in navigator) {
            try {
                const reg = await navigator.serviceWorker.ready; // Wait for active SW
                if (reg && reg.active) {
                    console.log('[App] Posting message to SW...');
                    ...
                    console.warn('[App] SW ready but no active worker found?');
                }
            } catch (e) {
                console.error('[App] SW Notification Logic Failed:', e);
            }
        }

        // 2. Fallback to classic Notification API (if SW failed or not present)
        try {
            console.log('[App] Using fallback new Notification() API.');
            new Notification(title, {
                body,
                icon: 'https://cdn-icons-png.flaticon.com/512/906/906334.png'
            });
        } catch (e) {
            console.error('[App] Fallback Notification Failed:', e);
        ...
        notifToggle.addEventListener('change', async (e) => {
            if (e.target.checked) {
                ...
                if (granted) {
                    ...
                    sendAppNotification("Notifications Enabled", "You will now receive task updates.");
                } else {
    ...
    document.getElementById('btn-test-notif').addEventListener('click', () => {
        if (localStorage.getItem('notifications') === 'true') {
            sendAppNotification("Test Notification", "If you see this, notifications are working! 🚀");
        } else {
    ...
    function checkDueTasks() {
        ...
        tasks.forEach(task => {
            if (!task.completed && !task.notified && task.date && task.time) {
                ...
                if (taskDue <= now) {
                    sendAppNotification("Task Reminder", `It's time for: ${task.title}`);
                    task.notified = true;
                    tasksUpdated = true;
                }
            }
    ...
    taskForm.addEventListener('submit', async (e) => {
        ...
        const newTask = {
            ...
            completed: false,
            notified: false // New field for notification tracking
        };
        await saveTask(newTask, true);
        sendAppNotification("Task Added", newTask.title);
        taskForm.reset();
