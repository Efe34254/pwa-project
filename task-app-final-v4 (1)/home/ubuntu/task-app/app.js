...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function sendAppNotification(title, body) {
        console.log('[App] Attempting to send notification:', title); // Debug
        if (localStorage.getItem('notifications') !== 'true') {
            console.log('[App] Notifications disabled in settings (localStorage)'); // Debug
            return;
        }

        if (Notification.permission === 'granted') {
            console.log('[App] Permission granted. Trying SW...'); // Debug
            // Try Service Worker first (better for PWA)
            if ('serviceWorker' in navigator) {
                try {
                    const reg = await navigator.serviceWorker.ready;
                    if (reg && reg.active) {
                        console.log('[App] SW Ready. Posting message...'); // Debug
                    ...
                    console.error('[App] SW Error:', e);
                }
            }
            // Fallback to standard notification
            console.log('[App] Fallback to new Notification()'); // Debug
            new Notification(title, { body, icon: 'https://cdn-icons-png.flaticon.com/512/906/906334.png' });
        } else {
            console.log('[App] Notification permission not granted:', Notification.permission); // Debug
        }
    }
